package com.matijasokol.hero_datasource_test.cache

import com.matijasokol.hero_datasource.cache.HeroCache
import com.matijasokol.hero_domain.Hero
import com.matijasokol.hero_domain.HeroRole
...
class HeroCacheFake(
    ...
    override suspend fun searchByRole(
        ...
        if (carry) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Carry) })
        if (escape) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Escape) })
        if (nuker) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Nuker) })
        if (initiator) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Initiator) })
        if (durable) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Durable) })
        if (disabler) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Disabler) })
        if (jungler) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Jungler) })
        if (support) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Support) })
        if (pusher) heros.addAll(db.heros.filter { it.roles.contains(HeroRole.Pusher) })
